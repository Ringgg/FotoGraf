...
CameraOrtho::CameraOrtho(int x, int y, double pxSize,
					   ...
					   : Camera(x,y, pos), pixelSize(pxSize), direciton(dir), up(u)
{
	...
	double halfsizeX = pxSize * sizeX * 0.5;
	double halfsizeY = pxSize * sizeY * 0.5;
...
void CameraOrtho::RenderPixel(double x, double y, bitmap_image& image)
{
	/*static Ray ray;
...
Color CameraOrtho::TestColor(Ray & ray)
{
	//static HitInfo cur;
	//static HitInfo best;
	...
	//if (best.d < 10000)
	//	return Color(1, 1, 1) * (Linear::Dot(ray.direction, best.n));
	//else				
		return backgroundColor;
}

Color CameraOrtho::TestSquare(Float3 lu, Float3 ld, Float3 ru, Float3 rd, double treshold = 0.25, double iterations = 3)
{
